...
def loadCam(args: GroupParams, id: int, cam_info: CameraInfo, resolution_scale: float) -> Camera:
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, FoVx=cam_info.FovX, FoVy=cam_info.FovY, image=gt_image, image_name=cam_info.image_name, uid=id, gt_alpha_mask=loaded_mask, data_device=args.data_device)

def cameraList_from_camInfos(cam_infos: List[CameraInfo], resolution_scale: float, args: GroupParams) -> List[Camera]:
    camera_list = []
...
def get_camera_mesh(extrinsic: Union[torch.Tensor, np.ndarray], intrinsic: Optional[Union[torch.Tensor, np.ndarray]]=None, is_c2w: bool=False, HW: Tuple[int]=None, camera_size: float=1.0, camera_color: Sequence[float]=[1.0, 1.0, 1.0], up_color: Sequence[float]=[1.0, 0.0, 0.0], right_color: Sequence[float]=[0.0, 1.0, 0.0]) -> trimesh.Trimesh:
    """format the extrinsic and intrinsic"""
    default_intrinsic = np.array([[1000.0, 0.0, 500.0], [0.0, 1000.0, 500.0], [0.0, 0.0, 1.0]]).astype(np.float32)
    extrinsic = np.asarray(extrinsic).astype(np.float32)
    intrinsic = np.asarray(intrinsic).astype(np.float32) if intrinsic is not None else default_intrinsic
    assert extrinsic.ndim == 2 and extrinsic.shape[0] in [3, 4] and (extrinsic.shape[1] == 4), f'extrinsic should be of shape [3/4, 4], but got {extrinsic.shape}'
    if extrinsic.shape[0] == 3:
        extrinsic = np.concatenate([extrinsic, np.array([[0, 0, 0, 1]])], axis=0)
    assert intrinsic.ndim == 2 and intrinsic.shape[0] == 3 and (intrinsic.shape[1] == 3), f'intrinsic should be of shape [3, 3], but got {intrinsic.shape}'
    pose = np.linalg.inv(extrinsic) if not is_c2w else extrinsic
    camera_faces = np.array([[0, 1, 2], [0, 2, 4], [0, 4, 3], [0, 3, 1], [5, 7, 6], [8, 10, 9]]).astype(np.int32)
    ...
    camera_vertices = np.array([[0.0, 0.0, 0.0], [tl[0], tl[1], 1.0], [tr[0], tr[1], 1.0], [bl[0], bl[1], 1.0], [br[0], br[1], 1.0], [tl[0] * 0.8, tl[1] * 1.1, 1.0], [tr[0] * 0.8, tr[1] * 1.1, 1.0], [0.0, tl[1] * 1.5, 1.0], [tr[0] * 1.05, tr[1] * 0.4, 1.0], [br[0] * 1.05, br[1] * 0.4, 1.0], [tr[0] * 1.2, 0.0, 1.0]], dtype=np.float32) * camera_size
...
def save_camera_mesh(extrinsics: Union[torch.Tensor, np.ndarray], intrinsics: Optional[Union[torch.Tensor, np.ndarray]]=None, is_c2w: bool=True, camera_size: Union[float, str]='auto', HW: Tuple[int]=None, camera_color: Sequence[float]=[1.0, 1.0, 1.0], up_color: Sequence[float]=[1.0, 0.0, 0.0], right_color: Sequence[float]=[0.0, 1.0, 0.0], verbose: bool=True, path: str='./cameras.ply') -> None:
    if extrinsics.ndim == 2:
        'just one camera'
        camera_size = 0.1 if camera_size == 'auto' else float(camera_size)
        camera_mesh = get_camera_mesh(extrinsics, intrinsics, is_c2w=is_c2w, camera_size=camera_size, HW=HW, camera_color=camera_color, up_color=up_color, right_color=right_color)
        ...
        for cam_id in range(num_cameras):
            ...
            cam_color = camera_color if not isinstance(camera_color[0], (Sequence, np.ndarray)) else camera_color[cam_id]
            u_color = up_color if not isinstance(up_color[0], (Sequence, np.ndarray)) else up_color[cam_id]
            r_color = right_color if not isinstance(right_color[0], (Sequence, np.ndarray)) else right_color[cam_id]
            camera_mesh = get_camera_mesh(extrinsic, intrinsic, is_c2w=is_c2w, camera_size=camera_size, HW=HW, camera_color=cam_color, up_color=u_color, right_color=r_color)
