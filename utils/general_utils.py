...
def inverse_sigmoid(x: torch.Tensor) -> torch.Tensor:
    x = x.clamp(1e-05, 1 - 1e-05)
    return torch.log(x / (1 - x))

def PILtoTorch(pil_image: Image.Image, resolution: Sequence[int]) -> torch.Tensor:
    resized_image_PIL = pil_image.resize(resolution, resample=Image.Resampling.BOX)
...
def get_expon_lr_func(lr_init: float, lr_final: float, lr_delay_steps: int=0, lr_delay_mult: float=1.0, max_steps: int=1000000) -> Callable:
...
def build_rotation(r: torch.Tensor) -> torch.Tensor:
    norm = torch.sqrt(r[:, 0] * r[:, 0] + r[:, 1] * r[:, 1] + r[:, 2] * r[:, 2] + r[:, 3] * r[:, 3]).clamp(min=1e-08)
    q = r / norm[:, None]
