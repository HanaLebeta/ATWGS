...
class GaussianModel:
    ...
    def capture(self) -> Tuple[int, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.optim.Optimizer, float]:
    ...
    def restore(self, model_args: Tuple[int, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict, float], training_args: Optional[GroupParams]=None):
    ...
    def _compute_3D_filter_coef(self):
        scales = self.get_scaling
        scales_square = torch.square(scales)
        det1 = scales_square.prod(dim=1)
        scales_after_square = scales_square + torch.square(self.filter_3D)
        det2 = scales_after_square.prod(dim=1)
        coef = torch.sqrt(det1 / det2.clamp(min=1e-20))
        return coef

    @property
    def get_opacity_with_3D_filter(self):
        opacity = self.opacity_activation(self._opacity)
        coef = self._compute_3D_filter_coef()
    ...
    def compute_3D_filter(self, cameras):
        ...
        Rs = torch.stack([torch.tensor(c.R, dtype=torch.float32) for c in cameras]).to(xyz.device)
        Ts = torch.stack([torch.tensor(c.T, dtype=torch.float32) for c in cameras]).to(xyz.device)
        focal_length = 0.0
        for i, camera in enumerate(cameras):
            xyz_cam = xyz @ Rs[i] + Ts[i][None, :]
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float) -> None:
        ...
        dist2 = torch.clamp_min(distCUDA2(fused_point_cloud), 1e-07)
    ...
    def training_setup(self, training_args: GroupParams) -> None:
        ...
        self.window_sig_accum = torch.zeros((self.get_xyz.shape[0], 1), device='cuda')
        self.window_sig_denom = torch.zeros((self.get_xyz.shape[0], 1), device='cuda')
    ...
    def save_ply(self, path: str) -> None:
        ...
        sh2rgb = eval_sh(0, self.get_features.transpose(1, 2).detach().cpu().numpy(), None)
        rgb = ((sh2rgb + 0.5).clip(0.0, 1.0) * 255.0).astype(np.uint8)
        ...
        attributes = np.concatenate((xyz, normals, rgb, f_dc, f_rest, opacities, scale, rotation, filter_3D), axis=1)
    ...
    def save_fused_ply(self, path):
        ...
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, rgb, f_dc, f_rest, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def prune_points(self, mask: torch.Tensor) -> None:
        ...
        if hasattr(self, 'window_sig_accum') and self.window_sig_accum.shape[0] == valid_points_mask.shape[0]:
            self.window_sig_accum = self.window_sig_accum[valid_points_mask]
            self.window_sig_denom = self.window_sig_denom[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz: torch.Tensor, new_features_dc: torch.Tensor, new_features_rest: torch.Tensor, new_opacities: torch.Tensor, new_scaling: torch.Tensor, new_rotation: torch.Tensor) -> None:
    ...
    def densify_and_split(self, grads: torch.Tensor, grad_threshold: float, grads_abs: torch.Tensor, grads_abs_threshold: torch.Tensor, scene_extent: float, N: int=2, dense: bool=False) -> None:
        ...
        if dense:
            ...
            selected_pts_mask_abs = torch.where(padded_grad_abs >= grads_abs_threshold, True, False)
    ...
    def densify_and_clone(self, grads: torch.Tensor, grad_threshold: float, grads_abs: torch.Tensor, grad_abs_threshold: torch.Tensor, scene_extent: float, dense: bool=False) -> None:
    ...
    def densify_and_prune(self, max_grad: float, min_opacity: float, extent: float, max_screen_size: int, dense: bool=False, window_signal_mode: bool=False, tau_window: float=0.0) -> None:
        if window_signal_mode:
            grads = self.window_sig_accum / self.window_sig_denom.clamp_min(1e-09)
            grads[grads.isnan()] = 0.0
            grads_abs = grads.clone()
            max_grad = tau_window
            ratio = (torch.norm(grads, dim=-1) >= max_grad).float().mean()
            Q = torch.quantile(grads_abs.reshape(-1), 1 - ratio) if 0.0 < ratio < 1.0 else grads_abs.max() + 1.0
        else:
    ...
    def add_window_signal_stats(self, window_signal: torch.Tensor) -> None:
        self.window_sig_accum += window_signal[:, 0:1]
        self.window_sig_denom += window_signal[:, 1:2]
