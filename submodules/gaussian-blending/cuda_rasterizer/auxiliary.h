...
#define ZNEAR (0.2f)
#define MIN_ALPHA (1.0f / 256.0f)
#define MIN_UV_DIFF (0.1f)
#define MAX_UV_DIFF (1000000.0f) 

#ifndef M_SQRT_PI_2
#define M_SQRT_PI_2 1.25331413731550025120788264240552f
#endif
#ifndef M_INV_SQRT_2
#define M_INV_SQRT_2 0.70710678118654752440084436210484f
#endif
...
__forceinline__ __device__ float gaussian_0th_moment(const float x)
{
    return M_SQRT_PI_2 * erff(x * M_INV_SQRT_2);
}
__forceinline__ __device__ float gaussian_1st_moment(const float x) 
{
    return -expm1f(-0.5f * x * x);
}
__forceinline__ __device__ float gaussian_2nd_moment(const float x)
{
    return gaussian_0th_moment(x) - expf(-0.5f * x * x) * x;
}
SUTIL_INLINE SUTIL_HOSTDEVICE float gaussian_0th_moment_backward(const float x)
{
