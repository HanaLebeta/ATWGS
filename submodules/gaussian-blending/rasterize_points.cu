...
RasterizeGaussiansCUDA(
    ...
    torch::Tensor out_window = torch::zeros({P, 2}, float_opts);  
    ...
    if(P != 0) {
        ...
        rendered = CudaRasterizer::Rasterizer::forward(
            ...
            out_window.contiguous().data_ptr<float>(),
    ...
    return std::make_tuple(rendered, out_color, out_window, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
RasterizeGaussiansBackwardCUDA(
    ...
    torch::Tensor dL_dcov = torch::zeros({P, 2, 2}, means3D.options());
