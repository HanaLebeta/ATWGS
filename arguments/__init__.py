        ...
        for key, value in vars(self).items():
            ...
            if shorthand:
                if t == bool:
                    ...
                    group.add_argument('--' + key, '-' + key[0:1], default=value, type=type(value[0]) if len(value) > 0 else float, nargs='+')
                else:
                    group.add_argument('--' + key, '-' + key[0:1], default=value, type=t)
            elif t == bool:
                group.add_argument('--' + key, default=value, action='store_true')
            elif t == list:
                group.add_argument('--' + key, default=value, type=type(value[0]) if len(value) > 0 else float, nargs='+')
            else:
...
class ModelParams(ParamGroup):
    ...
    def __init__(self, parser: ArgumentParser, sentinel=False) -> None:
        ...
        self.eval = False
        self.train_res = [1.0, 2.0, 4.0, 8.0]
